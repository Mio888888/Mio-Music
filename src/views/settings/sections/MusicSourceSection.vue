...
const currentSourceQualities = computed(() => {
  ...
  const selectedSource = (userInfo.value.supportedSources as any)?.[userInfo.value.selectSources]
...
const globalQualityOptions = computed(() => {
  ...
  const arrays = keys.map((k) => (sources as any)[k].qualitys || [])
...
const applyGlobalQuality = (q: string) => {
  ...
  Object.keys(sources).forEach((key) => {
    const arr = (sources as any)[key].qualitys || []
    if (arr.includes(q)) {
      if (!userInfo.value.sourceQualityMap) userInfo.value.sourceQualityMap = {}
  ...
  const arr = (sources as any)?.[currentKey]?.qualitys || []
...
const selectSource = (sourceKey: string) => {
  ...
  const source = (userInfo.value.supportedSources as any)?.[sourceKey]
  if (!userInfo.value.sourceQualityMap) userInfo.value.sourceQualityMap = {}
...
const onQualityChange = (value: any) => {
  if (currentSourceQualities.value.length > 0 && value >= 0 && value < currentSourceQualities.value.length) {
...
const getCurrentSourceName = () => {
  ...
  const source = (userInfo.value.supportedSources as any)?.[userInfo.value.selectSources]
  ...
  <div class="settings-section">
    <div v-if="hasPluginData" class="music-config-container">
        ...
        <div class="source-cards">
          ...
          >
            <div class="source-icon">
              <span style="font-size: 1.5em">{{ (source as any).name?.charAt(0) || '?' }}</span>
            </div>
            <div class="source-info">
              <div class="source-name">{{ (source as any).name }}</div>
              <div class="source-type">{{ (source as any).type || '音乐源' }}</div>
            </div>
            <div v-if="userInfo.selectSources === String(key)" class="source-check">&#10003;</div>
          </div>
        </div>
      </div>
      ...
      <div v-if="globalQualityOptions.length > 0" class="setting-group">
        <h3>全局音质（支持交集）</h3>
        <div class="quality-slider-container">
          <t-select v-model="globalQualitySelected" @change="(v: any) => applyGlobalQuality(v as string)">
    ...
    <div v-else class="plugin-prompt">
      ...
      <div class="prompt-content">
        ...
        <t-button theme="primary" @click="goPlugin">前往插件管理</t-button>
      </div>
    </div>
  </div>
</template>
...
.music-config-container {
  ...
  .source-card {
    display: flex; align-items: center; gap: 1rem; padding: 1rem;
    background: var(--td-bg-color-container); border: 2px solid var(--td-border-level-1-color);
    border-radius: 0.75rem; cursor: pointer; transition: all 0.2s ease;
    &:hover { border-color: var(--td-brand-color-3); box-shadow: 0 4px 6px -1px rgba(0,0,0,0.1); }
    &.active { border-color: var(--td-brand-color); background: var(--td-brand-color-1); box-shadow: 0 0 0 3px var(--td-brand-color-2); }
    .source-icon {
      width: 2.5rem; height: 2.5rem; background: var(--td-brand-color-1); border-radius: 50%;
      display: flex; align-items: center; justify-content: center; color: var(--td-brand-color);
    }
    .source-info { flex: 1;
      .source-name { font-weight: 600; font-size: 0.875rem; color: var(--td-text-color-primary); margin-bottom: 0.125rem; }
      .source-type { font-size: 0.75rem; color: var(--td-text-color-secondary); }
    }
    .source-check { color: var(--td-brand-color); font-size: 1.125rem; }
  }
  .quality-slider-container {
    background: var(--td-bg-color-page); padding: 1.5rem; border-radius: 0.75rem;
    border: 1px solid var(--td-border-level-1-color);
    .quality-slider { margin-bottom: 1rem; }
  }
  .quality-description { text-align: center; margin-top: 1rem;
    p { margin: 0.5rem 0;
      &:first-child { font-size: 1rem; font-weight: 600; color: var(--td-text-color-primary); }
      &.quality-hint { font-size: 0.875rem; color: var(--td-text-color-secondary); }
    }
  }
  .config-status { display: grid; grid-template-columns: 1fr 1fr; gap: 1rem;
    .status-item { display: flex; justify-content: space-between; align-items: center; padding: 1rem; background: var(--td-bg-color-page); border-radius: 0.5rem; border: 1px solid var(--td-border-level-1-color);
      .status-label { font-weight: 500; color: var(--td-text-color-secondary); font-size: 0.875rem; }
      .status-value { font-weight: 600; color: var(--td-text-color-primary); font-size: 0.875rem; }
    }
  }
}
.plugin-prompt {
  display: flex; align-items: center; gap: 1.5rem; padding: 2rem;
  background: var(--td-bg-color-container); border-radius: 1rem; border: 2px dashed var(--td-border-level-1-color);
  .prompt-icon {
  ...
  .prompt-content {
    flex: 1;
    h4 { color: var(--td-text-color-primary); margin: 0 0 0.5rem 0; font-size: 1.125rem; font-weight: 600; }
    p { color: var(--td-text-color-secondary); margin: 0 0 1.5rem 0; line-height: 1.5; }
  }
}
